...
export default function BookDetailPage({ book }: BookPageProps) {
  ...
  const IconWrapper = ({ children }: { children: React.ReactNode }) => (
    <span className="w-5 h-5 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 flex-shrink-0">
      {children}
    </span>
  );
    ...
    <>
      ...
      <div className="min-h-screen bg-[#f8f9fa] dark:bg-slate-900">
        <div className="max-w-6xl mx-auto px-2 sm:px-4 pt-24 sm:pt-32 pb-6">
                ...
                <div className="flex flex-col gap-1 sm:flex-row sm:flex-wrap sm:gap-x-3 sm:gap-y-1 text-sm sm:text-base text-slate-600 dark:text-slate-300">
                  {book.wordCount && (
                    <div className="flex items-center gap-2">
                      ...
                      <span><span className="font-medium">字数：</span>{book.wordCount}</span>
                    </div>
                  )}
                  {book.platform && book.platform !== "实体书" && (
                    <div className="flex items-center gap-2">
                      ...
                      <span><span className="font-medium">平台：</span>{book.platform}</span>
                    </div>
                  )}
                  {book.finishTime && (
                    <div className="flex items-center gap-2">
                      ...
                      <span><span className="font-medium">完结：</span>{book.finishTime}</span>
                    </div>
                  )}
                </div>
          ...
          <div className="mb-4 text-xs text-gray-500 dark:bg-slate-900 leading-relaxed space-y-1">
